...
class TestCheckin:
    async def test_checkin_creates_log(self, db_session, make_user, make_habit):
        ...
        log = await checkin(
            db=db_session, habit=habit, user=user, log_date=date.today()
        )
    ...
    async def test_checkin_duplicate_same_day_raises(
    ...
    async def test_checkin_different_days_ok(self, db_session, make_user, make_habit):
        ...
        log2 = await checkin(
            db=db_session, habit=habit, user=user, log_date=days_ago(1)
        )
...
class TestUndoCheckin:
    async def test_undo_checkin_removes_log(
    ...
    async def test_get_log_wrong_user_returns_none(
        self, db_session, make_user, make_habit, make_log
    ):
...
class TestTodayStatus:
    async def test_today_returns_all_active_habits(
        self, db_session, make_user, make_habit
    ):
        ...
        results = await get_today_status(
            db=db_session, user_id=user.id, today=date.today()
        )
    ...
    async def test_today_marks_completed_habits(
        self, db_session, make_user, make_habit, make_log
    ):
        from app.services.log_service import get_today_status
    ...
    async def test_today_marks_incomplete_habits(
        self, db_session, make_user, make_habit
    ):
        from app.services.log_service import get_today_status
    ...
    async def test_today_only_returns_user_habits(
        self, db_session, make_user, make_habit
    ):
        from app.services.log_service import get_today_status
        ...
        results_a = await get_today_status(
            db=db_session, user_id=user_a.id, today=date.today()
        )
        results_b = await get_today_status(
            db=db_session, user_id=user_b.id, today=date.today()
        )
...
class TestStats:
    async def test_stats_completion_rate_100(
        self, db_session, make_user, make_habit, make_log
    ):
        ...
        stats = await get_stats(
            db=db_session, habit=habit, from_date=days_ago(6), to_date=date.today()
        )
    ...
    async def test_stats_completion_rate_partial(
        self, db_session, make_user, make_habit, make_log
    ):
        from app.services.log_service import get_stats
    ...
    async def test_stats_weekly_average(
        self, db_session, make_user, make_habit, make_log
    ):
        from app.services.log_service import get_stats
        ...
        stats = await get_stats(
            db=db_session, habit=habit, from_date=days_ago(13), to_date=date.today()
        )

        # 8 completions over 2 weeks = 4.0 average
        assert stats["weekly_average"] == pytest.approx(4.0)

    async def test_stats_respects_date_range(
        self, db_session, make_user, make_habit, make_log
    ):
        from app.services.log_service import get_stats
        ...
        stats = await get_stats(
            db=db_session, habit=habit, from_date=days_ago(2), to_date=date.today()
        )
